...
def get_mask(ROI_Name, RS_filepath, CT_folderpath):
    ...
    RS_filepath: filepath to the RS file
    ...
    ds = pydicom.read_file(CT_folderpath + '/' + CT_slices[0])
...
def get_hu(CT_folderpath, mask, slice_numbers):
    ...
    for CT_file in CT_files:
        ds = pydicom.read_file(CT_folderpath + '/' + CT_file)
        num = round(ds.ImagePositionPatient[2],2)
        if num in slice_numbers:
            slice_location.append(ds.ImagePositionPatient[2])
    ...
    for i, contour_slice in enumerate(mask):
        ds = pydicom.read_file(CT_folderpath + '/' + CT_slices[i])
...
def get_HU_txt(RS_filepath, ROI_Name, CT_folderpath, output_folderpath, patient_x):
    ...
    #Assuming all CT images in the same folder are of the same shape and position    
    ct = pydicom.read_file(CT_folderpath + '/' + os.listdir(CT_folderpath)[0])
    ...
    f = open(output_folderpath + "/" + patient_x + "_" + ct.SeriesDate + ".txt",'w')
    f.write("Hounsfield Units: \n" + ', '.join(str(x) for x in hounsfield_units)+ "\n")
    
    #Save certain values to txt file
    f.write("Study Date: \n" + ct.StudyDate + "\n")
    f.write("Series Date: \n" + ct.SeriesDate + "\n")
    f.write("Patient's Birth Year: \n" + ct.PatientBirthDate[0:4] + "\n")
    f.write("Voxel Diameter (x, y, z): \n"  + ", ".join(str(x) for x in ct.PixelSpacing) + ", " + str(ct.SliceThickness) + "\n")
    f.close()

#Execution line:
#The function: get_HU_txt(RS_filepath, ROI_Name, CT_folderpath, output_folderpath, patient_x)
get_HU_txt('patient_alot/day20/RS.dcm', 'RECTUM', 'patient_alot/day20/CTSCAN', 'HU', 'patientlotsagas_day20')
#
#
